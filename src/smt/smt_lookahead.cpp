...
int find_best_var(std::vector<std::vector<double>> &X, int id, int thread) {
    ...
    expr_ref_vector lookahead::choose_rec(unsigned depth, 
        ...
        if(ctx.first_search){
            ...
            ctx.first_search = false;
            compare comp(ctx);
            std::sort(ctx.m_vars.begin(), ctx.m_vars.end(), comp);
            ...
            int budget = 250;
            for (bool_var v : ctx.m_vars) {
                if(budget-- <= 0) break;
                double num_occurrence = num_occurrences[v];
                ...
                std::vector<double> curr_x{
                    ctx.get_activity(v), static_cast<double>(ctx.get_assignment_time(v)),
                    ...
                    static_cast<double>(gained_clauses), score_param,   num_total,
                };
                ...
                result.push_back(ctx.bool_var2expr(v));
                ctx.get_pp_visited().reset();
            }
        }
        return result;
